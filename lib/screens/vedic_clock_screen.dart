  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(AppLocale.l('vedicClock'),
...
class _GhatiClockPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 60; i++) {
      ...
      if (isMajor) {
        final numR = R * 0.72;
        final label = i == 0 ? AppLocale.l('udaya') : '$i';
    ...
    // Labels below digits
    _txt(canvas, AppLocale.l('ghatiLabel'), cx, digitalY + digitFs * 0.9, Colors.white.withOpacity(0.4), labelFs, false);
    ...
    // Sunrise
    _txt(canvas, '☀ ' + AppLocale.l('udaya') + ' ' + sunriseStr, cx - R * 0.18, infoY,
      const Color(0xFFFFAB40), infoFs, true);
    // Sunset
    _txt(canvas, '🌙 ' + AppLocale.l('asta') + ' ' + sunsetStr, cx + R * 0.18, infoY,
